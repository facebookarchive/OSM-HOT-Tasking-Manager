...
export const TeamMembersStatsTable = ({ stats, teamName }) => {
  ...
  */
  ...
  const validatedKeys = ['username', 'tasksValidated', 'timeSpentValidating', 'averageValidationTime'];
  ...
  const validatedTableColumns = useMemo(() => [
    ...
    },
    {
      ...
      selector: row => new Date(row['averageValidationTime'] * 1000).toISOString().slice(11, 19),
      sortable: true,
      grow: 2,
      minWidth: '200px',
    },
  ...
  const ExportButton = (keys, title) => useMemo(() => {
    return (
      <Button
  ...
  return (
    ...
    <div className="cf shadow-4 pv3 ph2 bg-white">
      <Tabs>
        <TabList>
          <Tab><FormattedMessage {...messages.summary} /></Tab>
          <Tab><FormattedMessage {...messages.mapped} /></Tab>
          <Tab><FormattedMessage {...messages.validated} /></Tab>
        </TabList>

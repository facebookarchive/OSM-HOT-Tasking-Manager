...
class ProjectAdminService:
    ...
    def update_project(project_dto: ProjectDTO, authenticated_user_id: int):
        ...
        user = UserService.get_user_by_id(authenticated_user_id)
        username = user.name
        ...
        if project_dto.private:
            ...
            is_allowed_user = username in project_dto.allowed_usernames

        if is_admin or is_author or is_org_manager or is_team_member or is_allowed_user:
            project = ProjectAdminService._get_project_by_id(project_id)
